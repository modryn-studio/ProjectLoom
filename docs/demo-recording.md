...
1. A root card is created and the first prompt is typed + sent
2. After each AI response, the chat panel closes
3. You perform the manual action (branch or merge), then the next prompt fires
...
- **Escape** or the **Stop** button cancels at any point
- The floating **REC** badge in the top-right shows which step you're on
