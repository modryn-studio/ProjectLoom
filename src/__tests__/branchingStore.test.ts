...
describe('branching invariants', () => {
  ...
  it('does not erase messages when undo is triggered after updateConversation (structural-only undo)', () => {
    // Regression test: "create card → chat → Ctrl+Z → messages wiped"
    // Undo must be structural-only — it must never revert conversation.content.
    const card = makeConversation({
      id: 'card-1',
      content: [],
      position: { x: 0, y: 0 },
    });
    ...
    // Simulate a subsequent structural event (e.g. node drag) recording history with empty card
    useCanvasStore.getState().recordHistory();

    // Now simulate messages being added via updateConversation (no recordHistory call)
    const messages = [
      makeMessage('Hello', 'user'),
      makeMessage('World', 'assistant'),
    ];
    useCanvasStore.getState().updateConversation('card-1', {
      content: messages,
      metadata: {
        ...card.metadata,
        messageCount: 2,
    ...
    // Sanity check: messages are present
    expect(useCanvasStore.getState().conversations.get('card-1')?.content).toHaveLength(2);

    // Undo should NOT wipe the messages
    useCanvasStore.getState().undo();

    const afterUndo = useCanvasStore.getState().conversations.get('card-1');
    expect(afterUndo).toBeTruthy();
    expect(afterUndo?.content).toHaveLength(2);
    expect(afterUndo?.content[0]?.content).toBe('Hello');
    expect(afterUndo?.content[1]?.content).toBe('World');
    // Title should also be preserved
    expect(afterUndo?.metadata.title).toBe('Hello conversation');
  });

  it('restores message content when undoing a deleteConversation', () => {
    // Regression: deleting a card that has messages, then undoing, should restore
    // the card with its actual messages — not the empty creation-time snapshot.
    const card = makeConversation({
      id: 'card-del',
      content: [],
      position: { x: 0, y: 0 },
    ...
    // Simulate messages being added (no recordHistory called — same as real usage)
    const messages = [
      makeMessage('Question?', 'user'),
      makeMessage('Answer!', 'assistant'),
    ];
    useCanvasStore.getState().updateConversation('card-del', {
      content: messages,
      metadata: { ...card.metadata, messageCount: 2, title: 'My Chat' },
    });

    expect(useCanvasStore.getState().conversations.get('card-del')?.content).toHaveLength(2);

    // Delete the card
    useCanvasStore.getState().deleteConversation('card-del');
    expect(useCanvasStore.getState().conversations.has('card-del')).toBe(false);

    // Undo the deletion — card should come back WITH its messages
    useCanvasStore.getState().undo();

    const restored = useCanvasStore.getState().conversations.get('card-del');
    expect(restored).toBeTruthy();
    expect(restored?.content).toHaveLength(2);
    expect(restored?.content[0]?.content).toBe('Question?');
    expect(restored?.content[1]?.content).toBe('Answer!');
    expect(restored?.metadata.title).toBe('My Chat');

    // Node should also reflect the restored content
    const restoredNode = useCanvasStore.getState().nodes.find(n => n.id === 'card-del');
    expect(restoredNode?.data.conversation.content).toHaveLength(2);
  });
});
