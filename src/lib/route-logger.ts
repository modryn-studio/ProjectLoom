 ...
 *
 * Drop-in structured logging for Next.js API routes.
 * Produces visual separators + request IDs for easy terminal scanning.
 *
 * Usage:
 *   const log = createRouteLogger('my-route');
 *   export async function POST(req: Request) {
 *     const { reqId, start, end, error } = log.begin();
 *     try {
 *       log.info(reqId, 'Request received', { model, messageCount });
 *       // ... handler body ...
 *       return end(reqId, Response.json(result), { outputTokens });
 *     } catch (e) {
 *       return error(reqId, e);
 *     }
 *   }
 */
